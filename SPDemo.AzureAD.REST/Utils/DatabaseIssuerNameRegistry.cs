...
using SPDemo.AzureAD.REST.Models;
...
namespace SPDemo.AzureAD.REST.Utils
{
    public class DatabaseIssuerNameRegistry : ValidatingIssuerNameRegistry
    {
            ...
            if (newKeys || refreshTenant)
            {
                using (TenantDbContext context = new TenantDbContext())
                {
                    if (newKeys)
                    {
                        ...
                        foreach (string thumbprint in issuingAuthority.Thumbprints)
                        {
                            context.IssuingAuthorityKeys.Add(new TenantRegistrationModels.IssuingAuthorityKey { Id = thumbprint });
                        }
                    }
                    ...
                    if (refreshTenant)
                    {
                        ...
                        foreach (string issuer in issuingAuthority.Issuers)
                        {
                            ...
                            {
                                context.Tenants.Add(new TenantRegistrationModels.Tenant { Id = issuerId });
                            }
                        }
                    }
                    context.SaveChanges();
        ...
        public static bool TryAddTenant(string tenantId, string signupToken)
        {
            if (!ContainsTenant(tenantId))
            {
                using (TenantDbContext context = new TenantDbContext())
                {
                    ...
                    {
                        context.Tenants.Add(new TenantRegistrationModels.Tenant { Id = tenantId });
                        context.SaveChanges();
                        return true;
                    }
                    else
        ...
        public static void AddSignupToken(string signupToken, DateTimeOffset expirationTime)
        {
            using (TenantDbContext context = new TenantDbContext())
            {
                context.SignupTokens.Add(new TenantRegistrationModels.SignupToken
                {
                    Id = signupToken,
        ...
        public static void CleanUpExpiredSignupTokens()
        {
            ...
            using (TenantDbContext context = new TenantDbContext())
            {
                IQueryable<TenantRegistrationModels.SignupToken> tokensToRemove = context.SignupTokens.Where(token => token.ExpirationDate <= now);
